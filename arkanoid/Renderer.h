...
inline void drawLevelClear(TFT_eSprite& canvas, int wave, int score) {
    ...
    // Kutlama parcaciklari (static: bir kez hesapla, her frame ayni pozisyonda ciz)
    static int celebPx[20], celebPy[20];
    static uint16_t celebCol[20];
    static int lastCelebWave = -1;
    if (lastCelebWave != wave) {
        lastCelebWave = wave;
        for (int i = 0; i < 20; i++) {
            celebPx[i] = random(10, SCR_W - 10);
            celebPy[i] = random(20, SCR_H - 20);
            celebCol[i] = BRICK_COLORS[random(0, BRICK_ROWS)];
        }
    }
    for (int i = 0; i < 20; i++) {
        canvas.fillRect(celebPx[i], celebPy[i], 3, 3, celebCol[i]);
    }
}
