...
inline void drawPauseOverlay(TFT_eSprite &s) {
    ...
    s.fillRoundRect(25, 36, 110, 60, 5, COLOR_PANEL_BG);
    s.drawRoundRect(25, 36, 110, 60, 5, COLOR_FRAME);
    s.setTextSize(1);
    s.setTextColor(TFT_WHITE);
    const char* t = "PAUSED";
    s.setCursor(80 - (strlen(t) * 6) / 2, 42);
    s.print(t);
    const char* a = "[A] Continue";
    s.setCursor(80 - (strlen(a) * 6) / 2, 56);
    s.print(a);
    s.setTextColor(COLOR_TEXT_DIM);
    const char* b = "[B] Game Menu";
    s.setCursor(80 - (strlen(b) * 6) / 2, 68);
    s.print(b);
    const char* c = "[C] OS Menu";
    s.setCursor(80 - (strlen(c) * 6) / 2, 80);
    s.print(c);
}
