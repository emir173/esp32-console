...
void loop() {
    ...
    switch (state) {
        ...
        case PLAYING: {
            ...
            if (newLevel != level) {
                if (newLevel >= 2 && level < 2) obstacleSpawnMs = now;
                level = newLevel;
                levelChangeMs = now;
                needBgRender = true;
            ...
            if (level >= 2) {
                bool obstacleVisible = (obstacleSpawnMs == 0 || now - obstacleSpawnMs >= 2000)
                                     || ((now / 200) % 2 == 0);
                if (obstacleVisible) {
        ...
        case GAMEOVER:
            ...
            if (!digitalRead(BTN_C) && now - gameOverMs > 600) {
                returnToOS();
            }
            break;
        ...
        case PAUSE: {
            ...
            // Engel (Seviye 2+): pause'da da her zaman çiz (yanıp sönme sadece ilk 2s)
            if (level >= 2) {
                bool obstacleVisible = (obstacleSpawnMs == 0 || now - obstacleSpawnMs >= 2000)
                                     || ((now / 200) % 2 == 0);
                if (obstacleVisible) {
