...
void loop() {
    ...
    case ST_FINISH:
        if (!digitalRead(BTN_C)) { returnToOS(); break; }
        if (pressB) { playSound(NOTE_G4, 50); state = ST_TITLE; drawTitleScreen(canvas, bestLapTime); checkScreenshot(canvas); canvas.pushSprite(0, 0); break; }
